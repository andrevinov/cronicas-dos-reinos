# Fontes de Kara-Tur

Este arquivo registra as fontes usadas para Kara-Tur na campanha.

Kara-Tur é relevante principalmente por causa da origem de **Ren Kagehira**,
de **Masao Hirasawa**, da **Juppongatana**, dos aliados canônicos futuros de Ren,
da Ponte de Kozakura e de possíveis vínculos futuros entre Faerûn e os reinos
orientais de Forgotten Realms.

---

## Fonte principal

| Prioridade | Fonte | Uso na campanha | Arquivo local |
| --- | --- | --- | --- |
| Alta | Kara-Tur: The Eastern Realms, Forgotten Realms Campaign #1032, AD&D | Kozakura, Shou Lung, T'u Lung, Koryo, Wa, Tabot, cultura, facções, NPCs, lendas, itens e ganchos de aventura | `books/high_fr_adnd1e_tsr-1032_kara-tur-the-eastern-realms.pdf` |

---

## Documento interno canônico

| Prioridade | Fonte | Uso na campanha | Arquivo local |
| --- | --- | --- | --- |
| Alta, interna | Kara-Tur e Faerûn | Encaixe aprovado de Ren, Masao e Clã Kagehira na lacuna entre Kozakura em 1357 DR e a campanha em 1372 DR; composição canônica da Juppongatana; imagem e plano canônicos de Masao; relação geral entre Kara-Tur e Faerûn; limites do que Ren sabe sobre o Ocidente | `books/Kara-Tur e Faerûn.pdf` |
| Alta, interna | Aliados para Ren | Aliados canônicos futuros de Ren; ordem preferencial de aparição; ligação com Shou Lung, Kozakura, Tyr em Ravens Bluff, sobreviventes Kagehira e a divindade que condenou Masao | `books/Aliados para Ren.pdf` |
| Alta, interna | Ponte de Kozakura | Eixo futuro canônico de portal permanente entre Ravens Bluff e Kozakura, anomalias de comércio/pessoas, perda do controle exclusivo de Masao e surgimento futuro de Shin-Kozakura | `books/Ponte de Kozakura.pdf` |

Este documento não substitui o suplemento oficial como fonte de cenário. Ele
define como esta campanha preenche os espaços deixados pelo material publicado,
especialmente a crise política posterior ao retrato oficial de Kozakura.

---

## Forma de uso

O livro é uma fonte antiga de AD&D. Para esta campanha:

* cenário, nomes, regiões, culturas, facções, rotas, conflitos e itens lendários
  podem ser preservados;
* estatísticas, classes, níveis, magias e bônus numéricos devem ser convertidos
  para D&D 5e antes de entrar em jogo;
* elementos ligados diretamente a Ren e Masao devem ser reconciliados com os
  arquivos do personagem e do narrador;
* o tom deve evitar caricatura e exotização; usar Kara-Tur como região viva,
  politicamente complexa e com pessoas de objetivos próprios;
* material de bastidor que revele planos de Masao, membros da Juppongatana ou
  segredos do Clã Kagehira deve ficar em `narrador/`.

---

## Arquivos derivados

* `cenario/regioes/kara-tur/visao-geral.md`
* `cenario/regioes/kara-tur/kozakura.md`
* `cenario/regioes/kara-tur/faccoes-e-antagonistas.md`
* `cenario/regioes/kara-tur/itens-e-treinamentos-5e.md`
* `narrador/kara-tur-segredos.md`
* `narrador/aliados/`
* `narrador/ponte-de-kozakura/`
