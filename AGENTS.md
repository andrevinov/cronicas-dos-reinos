...
- fronteira/lote de pendências → `docs/task23-batch-world-boundary-resolution.md`; gate no `cronica preparar` → `docs/task24-pending-gate-cronica-preparar.md`;
...
**Barreira de pendências vive dentro de `cronica preparar`.** Não ler `runtime/mundo-pendencias.yaml` ritualisticamente antes do turno. Se `preparar` retornar `fase: bloqueada_pendencias_mundo`, **não narrar**: usar `resolver_fronteira.py preparar`, avaliar todos os itens na mesma inferência e enviar juntos os no-ops por `resolver_fronteira.py aplicar`; depois materializar somente `requer_resolucao` e repetir o mesmo `cronica preparar`. Evento canônico nunca aceita no-op. Candidato autônomo só aceita `sem_mudanca` por bloqueio canônico concreto; ausência de ação de Ren/fato novo não basta. Fallback de reparo: `endpoints.py pendencias`; `tipo: reavaliar_agente_leve` usa `agentes_leves.py concluir-noop <id>`; demais usam `barreira_mundo.py concluir <id>`. O writer repete a trava.
...
**Antes de narrar** intenção que comprime tempo — dormir, esperar, vigiar horas, viajar/trabalhar por período prolongado — consultar uma vez `endpoints.py fronteira --data ... --hora ...`. Se `interromper`, narrar até a fronteira e registrar/checkpointar; a continuação volta por `cronica preparar`, cujo gate impede atravessar pendências antes da resolução em lote. Não chamar a fronteira em turno curto.
