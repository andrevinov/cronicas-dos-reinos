...
Desde a Etapa 5, `estado/estado-atual.yaml` e `estado/tempo.yaml` descrevem **somente o presente**. Cronologia pertence a `sessoes/`; `historico/legado/` preserva depósitos antigos para auditoria e recuperação excepcional.

Desde a Etapa 6, relações, medidores de NPC e conhecimento são fragmentados. `estado/relacoes.yaml`, `estado/medidores-npcs.yaml` e `personagens/jogador/conhecimento.md` são **roteadores**, não depósitos. O presente vive em `estado/relacoes/<id>.yaml` e `estado/npcs/<id>.yaml`; conhecimento de Ren vive em fragmentos sob `personagens/jogador/conhecimento/`. Histórico completo de relação fica em `historico/relacoes/<id>.yaml`.
...
Runtime, índices e resultados de consulta não entram nessa hierarquia porque são projeções/roteadores. `historico/legado/` também não substitui fontes correntes.
...
Durante operação normal, preferir `ferramentas/contexto.py` a `cat`, `sed`, `rg` ou abertura integral de arquivos canônicos. A ferramenta resolve índice → fragmento e devolve somente o necessário.
...
- abrir todos os fragmentos de relações, NPCs ou conhecimento preventivamente;
...
- **L3:** `contexto.py buscar "termo"`, ainda sem transcrições/histórico frio;
- **L4:** `contexto.py buscar "termo" --historico`, histórico de relação, resumo/transcrição específica ou múltiplas fontes para resolver conflito;
- **L5:** fonte oficial externa/autorizada.

`historico/legado/` fica fora da escada normal e só deve ser aberto para auditoria de migração ou recuperação excepcional de algo ausente das fontes históricas próprias.
...
- seguir para fragmento/histórico direto somente quando a saída dirigida não resolver a lacuna;
...
Estado atual deve registrar **como as coisas estão agora**, não recontar como chegaram até ali. Relações atuais também não devem voltar a acumular cronologia; conhecimento novo deve ser incorporado à estrutura fragmentada e seus índices devem permanecer coerentes.
