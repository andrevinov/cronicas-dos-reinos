...
`narrador/indices/populacao-canonica.yaml` cobre **todo** `estado/npcs/index.yaml`, inclusive
NPCs sem relação com Ren. `narrador/indices/continuidade-autoral.yaml` cobre compromissos
...
`narrador/indices/estrutura.yaml` classifica autoridades, derivações, redirecionamentos,
