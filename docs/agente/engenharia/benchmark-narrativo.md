# Benchmark de memória, autonomia e consumo
...
Este protocolo constrói o instrumento; não antecipa correções posteriores de
compactação nem de execução autônoma. Um diagnóstico pode continuar mostrando
uma falha atual sem tornar a infraestrutura de medição incorreta.
...
A baseline registra sondas sobre os componentes já fundacionais, com hashes das
fontes e do catálogo. É fotografia histórica do instrumento, não de valores
vivos da campanha. Tokens nativos e qualidade narrada permanecem não medidos
nessa fotografia. A primeira baseline narrada exige rollouts reais destes
roteiros; a baseline de agosto não representa essa execução nova.
