# Memória relevante depois do estado efetivo
...
Medição local do mesmo caminho `contexto_core.command_npc` +
`fit_budget(8192, YAML)`, nas mesmas fixtures temporárias, comparando o código-base
de `67c0636badd81c032852d83f2b337850fb259c30` (blob original
`2aa34f4b8888128bf2a3e5a23ceecc495fcedcd4`) com a seleção atual:
...
Estes números medem **saída de componentes em fixtures**, não tokens nativos,
qualidade literária ou consumo de episódios narrados. Não há ensaio de IA novo
nesta medição. A comparação de episódios pertence ao benchmark narrativo; dados
ausentes não contam como economia. A integração verifica também a sonda de
reencontro, exigindo promessa e vínculo visíveis.
...
Não foram alterados o save, o cânone, as transcrições, os validadores, os
workflows, `cronica preparar` ou `cronica concluir`. A seleção de memória não
captura fatos novos da prosa nem injeta automaticamente memória dos participantes
no turno: essas responsabilidades pertencem à memória durável e à memória de cena.
