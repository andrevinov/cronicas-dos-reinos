...
O gate mantém side quests raras e reativas, mas é sensível à
...
não muda. O gate não reinicializa `narrador/tramas/oportunidades/estado.yaml`, não
reordena `restantes` e não infere seca retroativa.

## Pressão

Somente quando a ficha-base é `nada`, o gate consulta a pressão de seca.
...
persistência e orçamento continuam com uma única implementação. O gate não
...
A consulta de pressão lê somente `narrador/mundo/microeventos-locais/estado.yaml` e é
...
encontro continua sendo o mesmo `narrador/tramas/oportunidades/estado.yaml` já usado
