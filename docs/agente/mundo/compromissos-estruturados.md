...
O schema atual **não possui `local_id`**. Enquanto não houver integração
repo-aware com o Canonical Location Registry, o local fica no resumo/descrição.
Aceitar um identificador apenas porque ele “parece” snake_case reabriria uma
classe de inconsistência já eliminada. Uma extensão futura só deve adicionar
local estruturado passando pela mesma resolução canônica obrigatória.
