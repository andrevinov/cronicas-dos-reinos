...
Este mecanismo adiciona pressão operacional contra sequências longas de cenas
locais sem sequer um candidato de microevento. Ele **não mede tempo sem
aventura**, não lê transcrição e não decide se uma sessão foi “chata”.
...
O baralho-base continua exatamente `3 rotina : 1 microevento` e continua sem
reposição. A pressão não reseta nem reordena esse deck.
...
Quando um candidato é produzido, a sequência seca operacional é encerrada porque
o histórico grava `resultado: microevento`. Isso continua verdadeiro mesmo se o
narrador posteriormente descartar a manifestação por conflito com cânone forte.
Esse compromisso é intencional: o mecanismo regula a oferta do motor, não julga
semanticamente se a prosa final foi aventureira o suficiente.

O gate de sidequest pode consultar essa pressão sem transformar esta métrica em
verdade narrativa.
...
O resultado promovido continua passando pelo mesmo pool ecológico, pelos mesmos
guardrails e pelo mesmo `preparar → narrar → confirmar` dos microeventos.
...
Portanto o mecanismo adiciona ao hot path:
...
Isso reutiliza o contrato dos endpoints determinísticos sem criar novo formato
ou nova consulta.
