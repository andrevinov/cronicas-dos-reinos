...
interacoes.resolve_encounter_npc = lambda *a, **k: (_ for _ in ()).throw(
    AssertionError("não deveria resolver NPC")
)
interacoes.local_event = lambda *a, **k: (_ for _ in ()).throw(
    AssertionError("não deveria tocar local")
)
interacoes.encounter_event = lambda *a, **k: (_ for _ in ()).throw(
    AssertionError("não deveria tocar encontro")
)
interacoes._now = lambda *a, **k: (_ for _ in ()).throw(
    AssertionError("não deveria ler tempo")
)
...
opps.load_index = lambda *a, **k: (_ for _ in ()).throw(
    AssertionError("não deveria ler oportunidades")
)
...
class CenaMundoContextoTest(unittest.TestCase):
            ...
            "narrador/mundo/contextos-cena.yaml",
            {
                ...
                "candidatos": {
                    "presenca_shizune": {
                        ...
                        "prioridade": 100,
                        "min_coincidencias": 2,
                        "tags": [self.LOCAL, self.DOC, "assunto:escrituracao", self.REG],
                    },
                    "operacao_provas": {
                        "tipo": "operacao",
                        ...
                        "prioridade": 90,
                        "min_coincidencias": 2,
                        "tags": [self.DOC, self.REG, "assunto:provas"],
                    },
                    "direcao_ponte": {
                        "tipo": "direcao",
                        ...
                        "prioridade": 85,
                        "min_coincidencias": 2,
                        "tags": [self.DOC, self.REG, "assunto:kozakura"],
                    },
                },
            },
        )
        ...
        self._write(
            "narrador/arcos/parte_1.yaml",
            {
                ...
                "habilitacoes": {
                    ...
                    "antagonistas": ["shizune"],
                    "aliados": [],
                    "direcoes": ["ponte_de_kozakura"],
                },
                "linhas_operacionais": {
                    "impedir_consolidacao_de_provas": {
                        "objetivo": "impedir_provas",
                        "executores": ["shizune"],
                        "referencia": "plano",
                    }
                },
    ...
    def test_contexto_com_local_retorna_tres_classes_sem_oportunidades_ou_tempo(self):
        with (
            ...
            result = cena_mundo.open_scene(
                ...
                context_tags=[self.LOCAL, self.DOC, self.REG],
            )
    ...
    def test_assunto_sem_local_nao_fabrica_presenca(self):
        result = cena_mundo.open_scene(
            self.repo,
            scene_id="s009:documentos-remotos",
            context_tags=[self.DOC, self.REG],
        )
        self.assertEqual(result["presencas_contextuais"], [])
        self.assertEqual([x["id"] for x in result["operacoes_contextuais"]], ["impedir_consolidacao_de_provas"])
        self.assertEqual([x["id"] for x in result["direcoes_contextuais"]], ["ponte_de_kozakura"])
    ...
    def test_tags_sem_match_nao_exigem_controles_adicionais(self):
        result = cena_mundo.open_scene(
            self.repo,
            scene_id="s009:cozinha",
            context_tags=["assunto:cozinha", "assunto:comida"],
        )
        self.assertEqual(result["candidatos_contextuais"], [])
        self.assertEqual(result["fontes_lidas"], ["narrador/mundo/contextos-cena.yaml"])

    def test_tag_sem_namespace_falha_antes_de_qualquer_gatilho(self):
    ...
    def test_parser_preserva_tags_tipadas_repetidas(self):
        ...
        self.assertEqual(args.contexto_tag, [self.LOCAL, self.DOC])
