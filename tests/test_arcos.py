...
class ArcosTest(unittest.TestCase):
    ...
    def _contract(self, arc_id: str, title: str, *, antagonists: list[str], allies: list[str], directions: list[str], start: str, end: str) -> dict:
        return {
            ...
            "orquestracao": {
                "fontes": {
                    "plano_mestre": {"tipo": "documento_reservado", "arquivo": "narrador/elenco/masao/plano.md"},
                    "marcos_antagonistas": {"tipo": "documento_reservado", "arquivo": "narrador/elenco/juppongatana/marcos-de-aparicao.md"},
                },
    ...
    def _base_two_arcs(self):
        self._write(
            "narrador/tramas/arcos/index.yaml",
            {
                ...
                "arcos": {
                    "parte_1": {"titulo": "Parte 1", "ordem": 1, "arquivo": "narrador/tramas/arcos/parte_1.yaml", "proximo": "parte_2"},
                    "parte_2": {"titulo": "Parte 2", "ordem": 2, "arquivo": "narrador/tramas/arcos/parte_2.yaml", "proximo": None},
                },
            },
        )
        self._write(
        ...
        self._write("narrador/tramas/arcos/parte_1.yaml", self._contract("parte_1", "Parte 1", antagonists=["shizune", "kurobane"], allies=["shen"], directions=["ponte"], start="chegada_ren", end="perda_ponte"))
        self._write("narrador/tramas/arcos/parte_2.yaml", self._contract("parte_2", "Parte 2", antagonists=["anji"], allies=["joen"], directions=["shin"], start="perda_ponte", end="fim_parte_2"))
        self._write("campanha.yaml", "campanha: teste\n")
        self._write("narrador/elenco/masao/plano.md", "# Plano\n")
        self._write("narrador/elenco/juppongatana/marcos-de-aparicao.md", "# Marcos\n")

    def _references(self):
        self._write(
            "narrador/elenco/agentes/index.yaml",
            {"schema_agentes": 2, "agentes": {k: {"nome": k, "arquivo": f"narrador/elenco/agentes/{k}.yaml"} for k in ["masao", "shizune", "kurobane", "anji"]}},
        )
        self._write(
            "narrador/elenco/entradas/index.yaml",
            {"schema_entradas": 1, "candidatos": {"shen": {"nome": "Shen", "arquivo": "narrador/elenco/entradas/shen.yaml"}, "joen": {"nome": "Joen", "arquivo": "narrador/elenco/entradas/joen.yaml"}}},
        )
        self._write(
            "narrador/tramas/direcoes/index.yaml",
            {"schema_direcoes": 1, "direcoes": {"ponte": {"nome": "Ponte", "arquivo": "narrador/tramas/direcoes/ponte.yaml"}, "shin": {"nome": "Shin", "arquivo": "narrador/tramas/direcoes/shin.yaml"}}},
        )
        self._write("narrador/elenco/agentes/masao.yaml", {"id": "masao"})
        self._write(
            "narrador/elenco/agentes/shizune.yaml",
            {
                "id": "shizune",
        ...
        self._write("narrador/elenco/agentes/kurobane.yaml", {"id": "kurobane"})
        self._write(
            "narrador/elenco/agentes/anji.yaml",
            {
                "id": "anji",
                "metodos_operacionais": {
                    "linha_parte_2": [
        ...
        self._write("narrador/elenco/entradas/shen.yaml", "id: shen\n")
        self._write("narrador/elenco/entradas/joen.yaml", "id: joen\n")
        self._write("narrador/tramas/direcoes/ponte.yaml", "id: ponte\n")
        self._write("narrador/tramas/direcoes/shin.yaml", "id: shin\n")
    ...
    def test_indice_rejeita_transicao_para_tras(self):
        index = yaml.safe_load((self.repo / "narrador/tramas/arcos/index.yaml").read_text())
        index["arcos"]["parte_2"]["proximo"] = "parte_1"
        self._write("narrador/tramas/arcos/index.yaml", index)
    ...
    def test_plano_mestre_aponta_agente_e_fonte_sem_executar_nada(self):
        ...
        self.assertEqual(info["plano_mestre"]["fonte"], "narrador/elenco/masao/plano.md")
    ...
    def test_manifesto_resolve_fontes_sem_abrir_fragmentos(self):
        ...
        self.assertEqual(by_group["antagonistas"][0]["arquivo"], "narrador/elenco/agentes/shizune.yaml")
        self.assertEqual(by_group["aliados"][0]["arquivo"], "narrador/elenco/entradas/shen.yaml")
        self.assertEqual(by_group["direcoes"][0]["arquivo"], "narrador/tramas/direcoes/ponte.yaml")
        ...
        self.assertEqual(result["fontes_orquestradas"]["plano_mestre"]["arquivo"], "narrador/elenco/masao/plano.md")
    ...
    def test_resolver_peca_habilitada_devolve_ponteiros_sem_conteudo(self):
        ...
        self.assertEqual(result["fonte_especializada"], "narrador/elenco/agentes/shizune.yaml")
        self.assertEqual(result["referencias_estrategicas"]["plano_mestre"], "narrador/elenco/masao/plano.md")
        self.assertEqual(result["referencias_estrategicas"]["marcos_antagonistas"], "narrador/elenco/juppongatana/marcos-de-aparicao.md")
        self.assertNotIn("recursos", result)
        self.assertNotIn("plano_atual", result)
        self.assertNotIn("narrador/elenco/agentes/shizune.yaml", result["fontes_lidas"])
    ...
    def test_mudar_fragmento_do_agente_nao_exige_reescrever_contrato(self):
        contract_path = self.repo / "narrador/tramas/arcos/parte_1.yaml"
        before = contract_path.read_bytes()
        agent_path = self.repo / "narrador/elenco/agentes/shizune.yaml"
        ...
        self.assertEqual(resolved["fonte_especializada"], "narrador/elenco/agentes/shizune.yaml")
        self.assertEqual(contract_path.read_bytes(), before)

    def test_contrato_rejeita_conteudo_operacional_duplicado(self):
        path = self.repo / "narrador/tramas/arcos/parte_1.yaml"
    ...
    def test_validacao_detecta_fragmento_referenciado_ausente(self):
        (self.repo / "narrador/elenco/agentes/shizune.yaml").unlink()
    ...
    def test_validacao_detecta_fonte_orquestrada_ausente(self):
        (self.repo / "narrador/elenco/juppongatana/marcos-de-aparicao.md").unlink()
    ...
    def test_linhas_operacionais_listam_problemas_e_executores_sem_fragmentos(self):
        ...
        self.assertEqual(line["fonte_estrategica"], "narrador/elenco/masao/plano.md")
        self.assertFalse(any(source.endswith("/shizune.yaml") for source in result["fontes_lidas"]))
    ...
    def test_metodos_abrem_so_fragmento_do_executor_autorizado(self):
        ...
        self.assertEqual(result["fonte_agente"], "narrador/elenco/agentes/shizune.yaml")
    ...
    def test_executor_nao_autorizado_nao_abre_indice_nem_fragmento(self):
        (self.repo / "narrador/elenco/agentes/index.yaml").unlink()
        result = arcos.resolve_agent_methods(
            self.repo, "linha_parte_1", executor="kurobane"
        )
        self.assertFalse(result["executor_permitido"])
    ...
    def test_mesma_linha_pode_ter_traducoes_distintas_por_agente(self):
        ...
        self._write("narrador/tramas/arcos/parte_1.yaml", contract)
        self._write(
            "narrador/elenco/agentes/kurobane.yaml",
            {
                "id": "kurobane",
                "metodos_operacionais": {
                    "linha_parte_1": [
    ...
    def test_validacao_exige_traducao_para_cada_executor_de_linha(self):
        (self.repo / "narrador/elenco/agentes/shizune.yaml").write_text(
            "id: shizune\n", encoding="utf-8"
        )
    ...
    def test_linha_operacional_precisa_de_id_globalmente_unico(self):
        path = self.repo / "narrador/tramas/arcos/parte_2.yaml"
        contract = yaml.safe_load(path.read_text(encoding="utf-8"))
        ...
        self._write("narrador/tramas/arcos/parte_2.yaml", contract)
...
class ArcosBundleTest(unittest.TestCase):
    ...
    def test_parte_1_real_tem_linhas_operacionais_curadas_e_compactas(self):
        ...
        self.assertLessEqual(
            (ROOT / "narrador/tramas/arcos/parte_1_uma_ponte_para_kozakura.yaml").stat().st_size,
