...
class ContextSceneAlliesTest(unittest.TestCase):
    PRESSAO = "risco:pressao_shizune"
    DERROTA = "risco:derrota_grave"
    ...
    def setUp(self):
        ...
        self._write(
            "narrador/arcos/p1.yaml",
            {
                ...
                "linhas_operacionais": {
                    "linha": {"objetivo": "obj_linha", "executores": ["masao"], "referencia": "plano"}
                },
            },
        )
        self._write(
            ...
            "narrador/entradas/estado.yaml",
            {
                ...
                "candidatos": {
                    "shen": {
                        "estado": shen_state,
                        "antecipado": False,
                        "proxima_avaliacao": shen_due,
                        "historico_recente": ([{"acao": "abrir_janela_contextual"}] if shen_open and shen_due is None and shen_state == "latente" else []),
                    },
                    "joen": {
                        "estado": "latente",
                        "antecipado": False,
                        "proxima_avaliacao": joen_due,
                        "historico_recente": ([{"acao": "abrir_janela_contextual"}] if joen_open and joen_due is None else []),
                    },
                },
            },
        )
    ...
    def test_contexto_forte_retorna_shen_somente_com_janela_aberta(self):
        result = contexto_cena.select_candidates(self.repo, [self.PRESSAO], scene_id="s1")
        self.assertEqual([x["id"] for x in result["entradas"]], ["shen"])
        self.assertEqual(result["entradas"][0]["modo_avaliacao"], "avaliar_entrada_organica")
        self.assertNotIn("resultado", result["entradas"][0])
        self._state(
            shen_due={"data": "14 Eleasis, 1372 DR", "hora": "06:00"},
            shen_state="latente",
            level=6,
        )
        result = contexto_cena.select_candidates(self.repo, [self.PRESSAO], scene_id="s2")
        self.assertEqual(result["entradas"], [])

    def test_joen_nao_aparece_pela_tag_se_shen_ainda_e_foco(self):
        result = contexto_cena.select_candidates(self.repo, [self.DERROTA], scene_id="s3")
        self.assertEqual(result["entradas"], [])
    ...
    def test_shen_presente_level7_libera_joen_quando_janela_aberta(self):
        ...
        result = contexto_cena.select_candidates(self.repo, [self.DERROTA], scene_id="s4")
        self.assertEqual([x["id"] for x in result["entradas"]], ["joen"])
