...
class EntradasRepositoryTest(unittest.TestCase):
    ...
    def test_status_real_reflete_foco_sem_abrir_fragmentos(self):
        ...
        self.assertEqual(r["fontes_lidas"],["narrador/elenco/entradas/index.yaml","narrador/elenco/entradas/estado.yaml","runtime/contexto.yaml"])
    ...
    def test_consulta_de_jenilynn_e_fragmentada(self):
        ...
        self.assertIn("narrador/elenco/entradas/dame_jenilynn_leyland.yaml",r["fontes_lidas"])
        self.assertNotIn("narrador/elenco/entradas/shen_meihua.yaml",r["fontes_lidas"])

class EntradasSyntheticTest(unittest.TestCase):
    def setUp(self):
        self.tmp=tempfile.TemporaryDirectory(); self.repo=Path(self.tmp.name)
        for p in ("narrador/elenco/entradas","narrador/mundo","runtime","estado","fontes"): (self.repo/p).mkdir(parents=True,exist_ok=True)
        (self.repo/"fontes/canone.md").write_text("1. A;\n2. B.\nJanela A.\nJanela B.\n",encoding="utf-8")
        self.write("narrador/elenco/entradas/index.yaml",{"schema_entradas":1,"natureza":"reservado","cadencia_padrao_dias":3,"candidatos":{"a":{"nome":"Aliado A","ordem":1,"nivel_minimo_normal":6,"arquivo":"narrador/elenco/entradas/a.yaml"},"b":{"nome":"Aliado B","ordem":2,"nivel_minimo_normal":9,"arquivo":"narrador/elenco/entradas/b.yaml"}}})
        self.write("narrador/elenco/entradas/estado.yaml",{"schema_estado_entradas":1,"natureza":"controle_reservado","candidatos":{"a":{"estado":"latente","antecipado":False,"proxima_avaliacao":{"data":"11 Eleasis, 1372 DR","hora":"06:00"},"historico_recente":[]},"b":{"estado":"latente","antecipado":False,"proxima_avaliacao":None,"historico_recente":[]}}})
        for cid,name,order,level,ev in (("a","Aliado A",1,6,"1. A;"),("b","Aliado B",2,9,"2. B.")):
            self.write(f"narrador/elenco/entradas/{cid}.yaml",{"schema_entrada":1,"natureza":"reservado","id":cid,"nome":name,"ordem":order,"nivel_minimo_normal":level,"janela_preferencial":f"Janela {cid.upper()}.","gatilhos_fortes":["gatilho"],"forma_preferencial":"forma","funcao_imediata":"função","ancoras":[{"fonte":"fontes/canone.md","evidencia":ev}],"fontes_canonicas":["fontes/canone.md"]})
    ...
    def test_checkpoint_abre_janela_contextual_sem_pendencia(self):
        ...
        state=self.read("narrador/elenco/entradas/estado.yaml")["candidatos"]["a"]
        ...
        self.assertNotIn("narrador/elenco/entradas/a.yaml",r["fontes_lidas"])
    ...
    def test_nivel_baixo_adia_sem_criar_pendencia(self):
        ...
        self.assertEqual(self.read("narrador/mundo/estado.yaml")["pendencias"],[])
        self.assertEqual(self.read("narrador/elenco/entradas/estado.yaml")["candidatos"]["a"]["proxima_avaliacao"],{"data":"14 Eleasis, 1372 DR","hora":"06:00"})
    ...
    def test_janela_aberta_nao_duplica_historico_nem_pendencia(self):
        ...
        self.assertEqual(r["janela_contextual_aberta"],"a")
        self.assertEqual(self.read("narrador/mundo/estado.yaml")["pendencias"],[])
        history=self.read("narrador/elenco/entradas/estado.yaml")["candidatos"]["a"]["historico_recente"]
    ...
    def test_confirmar_primeiro_libera_segundo_no_proximo_amanhecer(self):
        ...
        st=self.read("narrador/elenco/entradas/estado.yaml"); self.assertEqual(st["candidatos"]["a"]["estado"],"presente")
    ...
    def test_antecipacao_fura_ordem_e_nivel_mas_e_unica(self):
        ...
        st=self.read("narrador/elenco/entradas/estado.yaml"); self.assertEqual(entradas.focus(entradas.load_index(self.repo),st),"b")
    ...
    def test_validacao_rejeita_evidencia_inventada(self):
        a=self.read("narrador/elenco/entradas/a.yaml"); a["ancoras"][0]["evidencia"]="não existe"; self.write("narrador/elenco/entradas/a.yaml",a)
