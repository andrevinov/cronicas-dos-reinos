# Ren Kagehira

Ren Kagehira é o personagem inicial de **Crônicas dos Reinos**.

Ele é um humano de **Kozakura**, em Kara-Tur, treinado como agente furtivo de um clã de sombras. Em termos mecânicos, usa **Dungeons & Dragons 5e**, classe **Monge**, nível 5, subclasse **Caminho da Sombra**.

---

## Conceito central

Ren é um solitário disciplinado, treinado para observar antes de agir, entrar e sair sem ser visto e sobreviver sem depender de um grupo fixo.

Ele chegou a **Ravens Bluff** seguindo o rastro de um traidor de seu clã. Esse traidor está ligado à morte de sua irmã mais velha e ao roubo de um selo secreto do clã.

Ren não veio a Faerûn para se tornar herói. Veio para encontrar uma pessoa. O problema é que Ravens Bluff é uma cidade onde uma busca privada pode rapidamente encostar em guildas, guardas, contrabandistas, nobres, templos e velhos segredos enterrados sob as ruas.

Sua origem vem do Clã Kagehira, uma rede shinobi de elite que servia ao shogunato de Kozakura como inteligência discreta. Ren e Aiko foram órfãos ainda crianças, sobreviveram anos na miséria e foram depois resgatados pelo tio, chefe do clã. Aiko se tornou shinobi antes dele e o ajudou a acompanhar o treinamento.

O inimigo central de Ren é **Masao Hirasawa**, velho criminoso de Kara-Tur, antigo monge segundo rumores, articulador do submundo de Kozakura e líder da **Juppongatana**, as Dez Espadas. Masao matou Aiko, roubou o Selo da Lua Velada e desapareceu para o oeste depois da queda do poder que quase consolidou em Kozakura.

---

## Função na campanha

Ren foi criado para uma campanha solo.

Ele deve ser capaz de:

* investigar sem chamar muita atenção;
* fugir de lutas ruins;
* vencer conflitos pequenos com velocidade;
* usar sombras, silêncio e disciplina para sobreviver;
* aceitar aliados temporários sem depender deles;
* manter objetivos próprios mesmo quando se envolve em problemas locais.

---

## Decisões mecânicas

* classe: Monge;
* nível inicial: 3;
* tradição monástica: Caminho da Sombra;
* ancestralidade: Humano variante;
* origem cultural: Kozakuran;
* background customizado: Exilado de Clã;
* talento inicial: Móvel;
* método de atributos: array solo melhorado `16, 15, 14, 13, 12, 10`;
* fonte mecânica principal: Livro do Jogador de D&D 5e.

---

## Entrada em Ravens Bluff

Ren chegou pelo porto, seguindo pistas fragmentadas sobre um homem de Kara-Tur que teria passado pela cidade sob identidade falsa.

Para conseguir cobertura legal, acesso às ruas e uma razão aceitável para fazer perguntas, Ren pretende se aproximar da **Night Watch** ou de algum contato menor da guarda.

Sua primeira cena provável envolve o porto, a guarda noturna ou uma carga suspeita ligada a viajantes vindos do leste.

---

## Pontos abertos

Estes detalhes podem ser definidos em jogo:

* quais facções de Ravens Bluff já protegem ou exploram o traidor;
* se algum sobrevivente do Clã Kagehira ainda apoia Ren ou o considera descartável;
* quais membros das Dez Espadas vieram para Faerûn, morreram ou se dispersaram;
* o que Masao roubou de uma divindade de Kara-Tur e quanto da lenda sobre o inferno é literal;
* se Masao veio para Faerûn fugindo, buscando algo sob a terra ou as duas coisas;
* quem ou o que salvou Ren depois da facada de Masao.
