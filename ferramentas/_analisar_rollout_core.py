...
WRITE_MARKERS = (
    ...
    "cronica registrar",
    "cronica confirmar",
    "cronica.py sessao checkpoint",
    "cronica.py sessao encerrar",
    "cronica.py sessao iniciar",
    "cronica.py sessao recuperar",
    "cronica.py progressao aplicar",
...
def _access_level_from_command(command: str) -> str | None:
    ...
    if re.search(r"\bcontexto\.py\b.*\bstatus\b", lower):
        return "L1"
    if re.search(
        r"\bcontexto\.py\b.*\b(?:retomada|cena|sessao|npc|local|relacao|recurso|conhecimento|regra|continuidade)\b",
        lower,
    ):
        return "L2"
    return None
