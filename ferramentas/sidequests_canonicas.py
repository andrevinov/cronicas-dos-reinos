...
A Task 31 aposentou o gate procedural. Esta camada é a nova origem operacional de
side quests: um roteador opaco já carregado em ``oportunidades/index.yaml`` aponta
para gates compactos por NPC; somente depois de todas as condições passarem o
arquivo reservado com os detalhes da missão é aberto.

Não há RNG, scheduler, scan geral de NPCs ou auto-oferta. Disponibilidade não é
fala, oferta não é aceite e Ren pode recusar toda missão. O lifecycle persistente
continua em ``oportunidades.py``.
"""
...
MAX_SUSPICION_CONDITIONS = 2
MAX_CONFIRMATION_CONDITIONS = 2
...
class CanonicalSidequestError(ValueError):
    """Contrato, gate ou detalhe de sidequest canônica inválido."""
...
def _quest_id(value: Any, label: str = "sidequest.id") -> str:
    ...
    if not QUEST_ID_RE.fullmatch(value):
        raise CanonicalSidequestError(
            f"{label} deve ser opaco no formato qsc- + 12 hexadecimais"
        )
    return value
...
def _router(index: dict[str, Any]) -> dict[str, Any]:
    ...
    if raw.get("schema_sidequests_canonicas") != 1 or raw.get("engine") != ENGINE_ID:
        raise CanonicalSidequestError("roteador de sidequests canônicas inválido")
    if raw.get("detalhes_somente_apos_gate") is not True:
        raise CanonicalSidequestError("roteador precisa exigir detalhe somente após gate")
    if raw.get("scheduler") != "proibido" or raw.get("rng") != "proibido":
        raise CanonicalSidequestError("sidequests canônicas não podem usar scheduler ou RNG")
    ...
    for npc_id, refs in mapping.items():
        ...
        if len(refs) > MAX_REFS_PER_NPC:
            raise CanonicalSidequestError(
                f"{npc_id}: máximo de {MAX_REFS_PER_NPC} referências canônicas"
            )
        for pos, item in enumerate(refs):
            item = _map(item, f"{npc_id}[{pos}]")
            extra = set(item) - {"id", "gate", "prioridade"}
            if extra:
                raise CanonicalSidequestError(
                    f"{npc_id}[{pos}] possui campos públicos demais: {', '.join(sorted(extra))}"
                )
            qid = _quest_id(item.get("id"), f"{npc_id}[{pos}].id")
            if qid in seen:
                raise CanonicalSidequestError(f"sidequest roteada duas vezes: {qid}")
            seen.add(qid)
            _repo_path(Path("/repo"), item.get("gate"), GATES_DIR)
            _integer(item.get("prioridade"), f"{qid}.prioridade", 0, 100)
    return raw


def route_for_npc(index: dict[str, Any], npc_id: str) -> list[dict[str, Any]]:
    """Retorna referências opacas sem abrir gate ou detalhe."""
    router = _router(index)
    refs = router["por_npc"].get(npc_id) or []
    return sorted(
        [
            {
                "id": item["id"],
...
def _absolute_instant(value: Any, label: str) -> mundo.WorldInstant:
    ...
    try:
        return mundo.parse_instant(_text(raw["data"], label + ".data"), _text(raw["hora"], label + ".hora"))
...
def _validate_gate_conditions(raw: Any, qid: str) -> dict[str, Any]:
    ...
    extra = set(conditions) - {"locais", "janela", "relacao", "conhecimento", "mundo", "identidade"}
    if extra:
        raise CanonicalSidequestError(
            f"{qid}.condicoes possui campos desconhecidos: {', '.join(sorted(extra))}"
        )

    locations = conditions.get("locais", [])
    if not isinstance(locations, list) or len(locations) > 6:
        raise CanonicalSidequestError(f"{qid}.condicoes.locais deve ser lista de até 6 ids")
    for local_id in locations:
        _text(local_id, f"{qid}.condicoes.locais")

    window = conditions.get("janela")
    if window is not None:
        window = _map(window, f"{qid}.condicoes.janela")
        if set(window) - {"inicio", "fim"}:
            raise CanonicalSidequestError(f"{qid}.condicoes.janela aceita somente inicio/fim")
        start = _absolute_instant(window["inicio"], f"{qid}.janela.inicio") if "inicio" in window else None
        end = _absolute_instant(window["fim"], f"{qid}.janela.fim") if "fim" in window else None
    ...
    if relation is not None:
        relation = _map(relation, f"{qid}.condicoes.relacao")
        allowed = {
            "afinidade_min",
            "afinidade_max",
            "confianca_min",
        ...
        if set(relation) - allowed:
            raise CanonicalSidequestError(f"{qid}.condicoes.relacao possui campo desconhecido")
        for key, value in relation.items():
            _integer(value, f"{qid}.relacao.{key}", 0, 10)

    knowledge = conditions.get("conhecimento", [])
    if not isinstance(knowledge, list) or len(knowledge) > MAX_KNOWLEDGE_CONDITIONS:
        raise CanonicalSidequestError(
    ...
    for pos, item in enumerate(knowledge):
        ...
        if not isinstance(item["presente"], bool):
            raise CanonicalSidequestError(f"{qid}.conhecimento[{pos}].presente deve ser booleano")

    world = conditions.get("mundo", [])
    if not isinstance(world, list) or len(world) > MAX_WORLD_CONDITIONS:
        raise CanonicalSidequestError(f"{qid}.mundo aceita até {MAX_WORLD_CONDITIONS} condições")
    for pos, item in enumerate(world):
        ...
        rel = Path(_text(item.get("arquivo"), f"{qid}.mundo[{pos}].arquivo"))
        if rel.is_absolute() or ".." in rel.parts or not rel.parts or rel.parts[0] not in WORLD_ROOTS:
            raise CanonicalSidequestError(f"{qid}.mundo[{pos}].arquivo fora dos roots permitidos")
        ...
        if operator not in WORLD_OPERATORS:
            raise CanonicalSidequestError(f"{qid}.mundo[{pos}].operador inválido: {operator}")
        if operator not in {"existe", "verdadeiro", "falso"} and "valor" not in item:
            raise CanonicalSidequestError(f"{qid}.mundo[{pos}] exige valor para {operator}")

    identity = conditions.get("identidade")
    if identity is not None:
        identity = _map(identity, f"{qid}.condicoes.identidade")
        ...
        if not isinstance(personas, list) or len(personas) > 3:
            raise CanonicalSidequestError(f"{qid}.identidade.persona_relacional deve ser lista")
        for item in personas:
            _text(item, f"{qid}.identidade.persona_relacional")
        suspicions = identity.get("suspeitas", [])
        if not isinstance(suspicions, list) or len(suspicions) > MAX_SUSPICION_CONDITIONS:
            raise CanonicalSidequestError(f"{qid}.identidade.suspeitas excede orçamento")
        for pos, item in enumerate(suspicions):
            ...
            if set(item) != {"observada", "possivel", "min_evidencias"}:
                raise CanonicalSidequestError(f"{qid}.identidade.suspeitas[{pos}] inválida")
            _text(item["observada"], "observada")
            _text(item["possivel"], "possivel")
            _integer(item["min_evidencias"], "min_evidencias", 1, identidades.MAX_EVIDENCE)
        confirmations = identity.get("confirmacoes", [])
        if not isinstance(confirmations, list) or len(confirmations) > MAX_CONFIRMATION_CONDITIONS:
            raise CanonicalSidequestError(f"{qid}.identidade.confirmacoes excede orçamento")
        for pos, item in enumerate(confirmations):
            ...
            if set(item) != {"observada", "identidade", "presente"}:
                raise CanonicalSidequestError(f"{qid}.identidade.confirmacoes[{pos}] inválida")
            _text(item["observada"], "observada")
            _text(item["identidade"], "identidade")
            if not isinstance(item["presente"], bool):
                raise CanonicalSidequestError("confirmacoes.presente deve ser booleano")
    return conditions


def _load_gate(repo: Path, ref: dict[str, Any]) -> tuple[dict[str, Any], str]:
    path = _repo_path(repo, ref.get("gate"), GATES_DIR)
    data = _map(_load(path), str(path.relative_to(repo)))
    qid = _quest_id(data.get("id"))
    if data.get("schema_gate_sidequest_canonica") != 1 or data.get("natureza") != "reservado":
        raise CanonicalSidequestError(f"{qid}: gate canônico inválido")
    if qid != ref.get("id") or data.get("npc_id") != ref.get("npc_id"):
    ...
    if extra:
        raise CanonicalSidequestError(f"{qid}: gate contém conteúdo narrativo/extra proibido")
    return data, path.relative_to(repo).as_posix()
    ...
    if isinstance(value, dict):
        for key, item in value.items():
            ...
            if normalized in FORBIDDEN_AGENCY_KEYS:
                raise CanonicalSidequestError(f"{path}.{key}: detalhe não pode controlar Ren")
            _agency_scan(item, path=f"{path}.{key}")
    elif isinstance(value, list):
        for pos, item in enumerate(value):
            _agency_scan(item, path=f"{path}[{pos}]")
...
def _load_detail(repo: Path, gate: dict[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    data = _map(_load(path), str(path.relative_to(repo)))
    qid = _quest_id(data.get("id"))
    if data.get("schema_sidequest_canonica") != 1 or data.get("natureza") != "reservado":
        raise CanonicalSidequestError(f"{qid}: detalhe canônico inválido")
    if qid != gate["id"] or data.get("npc_id") != gate["npc_id"]:
    ...
    if kind not in oportunidades.VALID_TYPES:
        raise CanonicalSidequestError(f"{qid}: tipo de missão inválido: {kind}")
    ...
    if offer.get("recusa_permitida") is not True:
        raise CanonicalSidequestError(f"{qid}: toda sidequest canônica precisa permitir recusa")
    ...
    _agency_scan(data)
    return copy.deepcopy(data), path.relative_to(repo).as_posix()


class _Context:
    def __init__(self, repo: Path, supplied_now: mundo.WorldInstant | None):
        self.repo = repo
        self.supplied_now = supplied_now
        self.sources: list[str] = []
        self._now: mundo.WorldInstant | None = supplied_now
    ...
    def add(self, *sources: str) -> None:
        self.sources = list(dict.fromkeys([*self.sources, *[item for item in sources if item]]))
    ...
    def pending(self) -> list[dict[str, Any]]:
        if self._pending is None:
            try:
                ...
                raise CanonicalSidequestError(f"não foi possível ler overlay pendente: {exc}") from exc
    ...
    def npc_payload(self, npc_id: str) -> dict[str, Any]:
        ...
        if self._npc_index is None:
            self._npc_index = _map(_load(self.repo / estado_relacional.NPC_INDEX), str(estado_relacional.NPC_INDEX))
            if not isinstance(self._npc_index.get("npcs"), dict):
                raise CanonicalSidequestError("índice de NPCs inválido para sidequest canônica")
        ...
        if not isinstance(entry, dict) or not isinstance(entry.get("arquivo"), str):
            raise CanonicalSidequestError(f"{npc_id}: NPC não possui fragmento operacional")
        rel = entry["arquivo"]
        ...
        try:
            ...
            raise CanonicalSidequestError(f"{npc_id}: overlay relacional inválido: {exc}") from exc
        self.add(rel)
    ...
    def world_doc(self, rel: str) -> Any:
        ...
        path = Path(rel)
        if path.is_absolute() or ".." in path.parts or not path.parts or path.parts[0] not in WORLD_ROOTS:
            raise CanonicalSidequestError(f"fonte de mundo fora do escopo: {rel}")
        data = _load(self.repo / path)
...
def _lifecycle_allows(ctx: _Context, index: dict[str, Any], quest_id: str) -> tuple[bool, str]:
    state = ctx.state(index)
    mid = mission_id(quest_id)
    existing = state["missoes"].get(mid)
    if isinstance(existing, dict):
        if existing.get("quest_id") != quest_id or existing.get("origem") != "sidequest_canonica":
            raise CanonicalSidequestError(f"colisão de mission_id canônico: {mid}")
        if existing.get("estado") == "recusada" and existing.get("pode_reabrir") is True:
            mode = "reabertura"
        else:
...
def _relation_allows(ctx: _Context, npc_id: str, raw: Any) -> bool:
    ...
    try:
        ...
        raise CanonicalSidequestError(str(exc)) from exc
    checks = {
        "afinidade_min": (projection["afinidade"], lambda a, b: a is not None and a >= b),
        "afinidade_max": (projection["afinidade"], lambda a, b: a is not None and a <= b),
        "confianca_min": (projection["confianca"], lambda a, b: a is not None and a >= b),
        "confianca_max": (projection["confianca"], lambda a, b: a is not None and a <= b),
        "risco_min": (projection["risco_percebido"], lambda a, b: a is not None and a >= b),
        "risco_max": (projection["risco_percebido"], lambda a, b: a is not None and a <= b),
    }
    return all(checks[key][1](checks[key][0], value) for key, value in relation.items())
    ...
    for record in records:
        for delta in record.get("deltas") or []:
            ...
            if not isinstance(target, str) or not target.startswith("conhecimento"):
                continue
            if needle in contexto_core.normalize(yaml.safe_dump(delta.get("valor"), allow_unicode=True)):
                return True
    return False
...
def _knowledge_allows(ctx: _Context, raw: Any) -> bool:
    for item in raw or []:
        ...
        ctx.add(rel)
        found = contexto_core.normalize(item["termo"]) in contexto_core.normalize(body)
        if not found:
            found = _pending_knows(ctx.pending(), item["termo"])
        if found is not bool(item["presente"]):
            return False
    return True
...
def _compare(actual: Any, operator: str, expected: Any = _MISSING) -> bool:
    if operator == "existe":
        ...
        return (actual is not _MISSING) is wanted
    ...
    raise CanonicalSidequestError(f"operador de mundo desconhecido: {operator}")


def _world_allows(ctx: _Context, raw: Any) -> bool:
    for item in raw or []:
        rel = str(item["arquivo"])
        actual = _get_path(ctx.world_doc(rel), str(item["caminho"]))
...
def _identity_allows(ctx: _Context, npc_id: str, raw: Any) -> bool:
    ...
    try:
        ...
        state = identidades.validate_state(payload.get(identidades.STATE_FIELD), registry)
    ...
    for item in identity.get("suspeitas") or []:
        if suspicions.get((item["observada"], item["possivel"]), 0) < item["min_evidencias"]:
            return False
...
def _evaluate_gate(
    ...
    *,
    ...
    allowed_places = list(conditions.get("locais") or [])
    if allowed_places and local_id not in allowed_places:
        return False, "local", gate

    window = conditions.get("janela")
    if isinstance(window, dict):
        current = ctx.now()
        if "inicio" in window and current.minute < _absolute_instant(window["inicio"], "janela.inicio").minute:
            return False, "data", gate
        if "fim" in window and current.minute > _absolute_instant(window["fim"], "janela.fim").minute:
            return False, "data", gate

    lifecycle_ok, mode = _lifecycle_allows(ctx, index, gate["id"])
    if not lifecycle_ok:
        return False, mode, gate
    if not _relation_allows(ctx, gate["npc_id"], conditions.get("relacao")):
        return False, "relacao", gate
    if not _knowledge_allows(ctx, conditions.get("conhecimento")):
        return False, "conhecimento", gate
    if not _world_allows(ctx, conditions.get("mundo")):
        return False, "mundo", gate
    if not _identity_allows(ctx, gate["npc_id"], conditions.get("identidade")):
        return False, "identidade", gate
    return True, mode, gate
...
def select_from_refs(
    ...
    *,
    ...
    """Avalia referências já roteadas; abre no máximo um detalhe secreto."""
    ...
    for ref in ordered:
        evaluated += 1
        eligible, mode, gate = _evaluate_gate(ctx, index, ref, local_id=local_id)
        if not eligible:
            if diagnostics:
        ...
        return {
            ...
            "sidequest": {
                ...
                "regra": (
                    "disponibilidade não é fala nem aceite; o NPC pode oferecer na cena, "
                    "mas Ren decide aceitar, adiar ou recusar"
                ),
            },
            "gates_avaliados": evaluated,
...
def evaluate_for_npc(
    ...
    *,
    ...
    result["fontes_lidas"] = list(dict.fromkeys([*sources, *(result.get("fontes_lidas") or [])]))
    return result


def _find_ref(index: dict[str, Any], npc_id: str, quest_id: str) -> dict[str, Any]:
    qid = _quest_id(quest_id)
...
def _mission_window(detail: dict[str, Any], current: mundo.WorldInstant) -> dict[str, Any]:
    try:
        return oportunidades._window_at({"janela": detail["janela"]}, current)
    except oportunidades.OpportunityError as exc:
        raise CanonicalSidequestError(str(exc)) from exc
...
def offer(
    ...
    *,
    ...
    """Materializa oferta canônica após revalidar o mesmo gate determinístico."""
    try:
        index = oportunidades.load_index(repo)
    except oportunidades.OpportunityError as exc:
        raise CanonicalSidequestError(str(exc)) from exc
    ...
    selected = select_from_refs(repo, [ref], local_id=local_id, now=now)
    if selected.get("resultado") != "sidequest_canonica_disponivel":
        raise CanonicalSidequestError("sidequest não está canonicamente elegível para oferta")
    sidequest = selected["sidequest"]
    detail = sidequest["detalhe"]
    if detail["id"] != quest_id:
        raise CanonicalSidequestError("seleção canônica divergente da oferta solicitada")

    try:
        state = oportunidades.load_state(repo, index)
    except oportunidades.OpportunityError as exc:
        raise CanonicalSidequestError(str(exc)) from exc
    sources.extend([*(selected.get("fontes_lidas") or []), oportunidades.STATE.as_posix()])
    current = now
    if current is None:
        try:
    ...
    if active >= budget["max_ativas"] or opened >= budget["max_em_aberto"]:
        raise CanonicalSidequestError("orçamento de sidequests não permite nova oferta")
    ...
    mission = {
        ...
        "janela": _mission_window(detail, current),
        "pode_reabrir": detail["pode_reabrir"],
        "consequencia_sem_ren": detail["consequencia_sem_ren"],
        "fonte_canonica": ref["gate"],
        "fonte_detalhe": str(_repo_path(repo, detail_source := _load_gate(repo, ref)[0]["detalhe"], DETAILS_DIR).relative_to(repo)),
    ...
    return {
        ...
        "missao": mission,
        "recusa_permitida": True,
        "proximo_passo": f"oportunidades.py responder {mid} aceitar|adiar|recusar",
        "fontes_lidas": list(dict.fromkeys(sources)),
    }
...
def check(repo: Path) -> dict[str, Any]:
    ...
    try:
        ...
        npc_index = _map(_load(repo / estado_relacional.NPC_INDEX), str(estado_relacional.NPC_INDEX))
        known_npcs = set(_map(npc_index.get("npcs"), "estado/npcs/index.yaml.npcs"))
                ...
                try:
                    ...
                    for local_id in conditions.get("locais") or []:
                        ...
                        if resolved["local_id"] != local_id:
                            raise CanonicalSidequestError(f"{gate['id']}: local de gate deve usar id canônico")
                    for item in conditions.get("conhecimento") or []:
                        path = _repo_path(repo, item["arquivo"], contexto_core.KNOW_ROOT)
                        if not path.is_file():
                            raise CanonicalSidequestError(f"{gate['id']}: fonte de conhecimento ausente")
                    for item in conditions.get("mundo") or []:
                        if not (repo / str(item["arquivo"])).is_file():
                            raise CanonicalSidequestError(f"{gate['id']}: fonte de mundo ausente: {item['arquivo']}")
                    identity = conditions.get("identidade") or {}
                    for persona in identity.get("persona_relacional") or []:
                        if persona not in valid_identities:
                            raise CanonicalSidequestError(f"{gate['id']}: persona relacional desconhecida: {persona}")
                    for item in identity.get("suspeitas") or []:
                        if item["observada"] not in valid_identities or item["possivel"] not in valid_identities:
                            raise CanonicalSidequestError(f"{gate['id']}: identidade de suspeita desconhecida")
                    for item in identity.get("confirmacoes") or []:
                        if item["observada"] not in valid_identities or item["identidade"] not in valid_identities:
                            raise CanonicalSidequestError(f"{gate['id']}: identidade de confirmação desconhecida")
                    _load_detail(repo, gate)  # manutenção fria: valida sem expor conteúdo
                except (CanonicalSidequestError, locais.LocationError) as exc:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    evaluate = sub.add_parser("avaliar", help="avalia disponibilidade para um NPC sem escrever")
    ...
    offer_parser = sub.add_parser("oferecer", help="revalida e registra oferta canônica")
    ...
    sub.add_parser("check", help="valida roteador, gates e detalhes sem expor segredos")
    return parser
