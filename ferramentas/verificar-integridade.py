...
try:
    ...
    from ferramentas import adversarios, ameacas, continuidade_autoral, dungeons, estrutura_narrador, populacao
...
def validate(repo: Path, baseline: Path | None = None) -> list[str]:
    ...
    dungeon_result = dungeons.validate_repo(repo)
    errors.extend(
        f"dungeons preparadas: {error}" for error in dungeon_result["erros"]
    )
