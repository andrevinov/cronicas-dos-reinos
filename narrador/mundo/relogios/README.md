...
Os relógios de `narrador/mundo/relogios/` não são agentes e não tomam decisões. Eles medem **pressão/progresso produzido por uma operação** ou preservam a **consequência já materializada** quando o limite foi alcançado.
...
`narrador/mundo/relogios/vinculos.yaml` é um roteador pequeno, derivado dos fragmentos. Ele permite descobrir pressões por agente sem abrir todos os relógios:
